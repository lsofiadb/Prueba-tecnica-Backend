...
public class VehicleService {
    public Position getLocation(ArrayList<Satellite> satellites) {
        ...
        double c = 2 * (x3 - x2);
        double d = 2 * (y3 - y2);
        double e = (Math.pow(r1, 2) - Math.pow(r2, 2) - Math.pow(x1, 2) + Math.pow(x2, 2) - Math.pow(y1, 2) + Math.pow(y2, 2));
        ...
        if(Math.abs((a*d)-(b*c)) >=Math.pow(10,-4)){
            ...
            double equation1 = (Math.pow((x - x1), 2) + Math.pow((y - y1), 2));
            double equation2 = (Math.pow((x - x2), 2) + Math.pow((y - y2), 2));
            double equation3 = (Math.pow((x - x3), 2) + Math.pow((y - y3), 2));
            double equality1 = Math.pow(r1, 2) - equation1;
            double equality2 = Math.pow(r2, 2) - equation2;
            double equality3 = Math.pow(r3, 2) - equation3;

            /*--------ERROR RANGE--------*/
            if (Math.abs(equality1) < Math.pow(10, -2) & Math.abs(equality2) < Math.pow(10, -2) & Math.abs(equality3) < Math.pow(10, -2)) {
                vehiclePosition.setX(roundNumber(x, 1));
                vehiclePosition.setY(roundNumber(y, 1));
    ...
    public double roundNumber(double number, int digitsNumber) {
        double result = number *  Math.pow(10, digitsNumber);
        result = Math.round(result);
        result = result / Math.pow(10, digitsNumber);
        return result;
    }
